...
function runSpup() {
	webApiClient.getSystemInfo(deploySpup, function(msg) {
		alert(msg);
	});
	return false;
}
...
function deploySpup() {
	var content = packageSpup("my-app", "<?= $a ?>");
	var baseUrl = "http://default-server/test";
	webApiClient.applicationDeploy(content, baseUrl, showSpup, errorSpup);
}
