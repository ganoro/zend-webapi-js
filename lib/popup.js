...
function getSpup() {
	var zip = new JSZip();
	var deploymentDescriptor = "<package xmlns=\"http://www.zend.com/server/deployment-descriptor/1.0\" version=\"1.0\"><name>{0}</name><version><release>1.0.0</release></version><appdir>data</appdir><scriptsdir>scripts</scriptsdir></package>";
	zip.add("deployment.xml", deploymentDescriptor);
	zip.folder("data").add("index.php", codeMirror.text());
	return zip.generate();
}
