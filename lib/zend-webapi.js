var webApiClient = {
	...
	getRequest : function(url, params, _success, _error) {
		...
		$.get({
			...
			beforeSend: function(jqXHR, settings) {
				// alert(settings.headers['Host']);
				jqXHR.setRequestHeader('Date', this.date);
				jqXHR.setRequestHeader('Host', 'royganor.my.phpcloud.com');
				jqXHR.setRequestHeader('User-Agent', 'my-user');
				return true;
			},
			success : function(data, textStatus) {
				try {
